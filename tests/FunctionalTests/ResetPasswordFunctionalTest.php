...
class ResetPasswordFunctionalTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        TestProcessHelper::runNow(sprintf('git clone %s %s/bundle', $rootPath, $cachePath), $cachePath);

        // Install Symfony Skeleton
        TestProcessHelper::runNow(sprintf('composer create-project symfony/skeleton %s --prefer-dist', $this->appPath), $cachePath);

        // Setup the app as a "webapp" (similar to symfony new --webapp)
        TestProcessHelper::runNow('composer require symfony/webapp-pack --prefer-dist', $this->appPath);
        ...
        TestProcessHelper::runNow('composer require symfonycasts/reset-password-bundle', $this->appPath);
        ...
        TestProcessHelper::runNow('bin/console make:user --is-entity --with-password -n --identity-property-name email User ', $this->appPath);
        ...
        // Setup persistence
        TestProcessHelper::runNow('bin/console d:s:create', $this->appPath);
    }

    public function testApp(): void
    {
        TestProcessHelper::runNow('bin/phpunit', $this->appPath);
